...
Template.addService.events ({
	...
	'submit #addServiceForm':function(e){
		e.preventDefault();
		var comp={};
		valid=true;
		comp.name=$('#name').val();
		comp.cost=$('#cost').val();
		comp.minimun=$('#min').val();
		comp.available=$('#available').val();
		comp.duration=$('#duration').val();
		comp.description=$('#description').val();
		comp.noInclude=$('#noInclude').val().split(',');
		comp.include=$('#include').val().split(',');
